...
<?php $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; ?>
	...
	<?php } else { ?>
			...
			<td align="center" valign="middle">
				...
				<b class="postauthor"<?php if ($_postrow_val['POST_AUTHOR_COLOUR']) {  ?> style="color: <?php echo $_postrow_val['POST_AUTHOR_COLOUR']; ?>"<?php } ?>><div style="vertical-align: -4px; display: inline;"><?php echo $_postrow_val['RANK_IMG']; ?></div> <?php echo $_postrow_val['POST_AUTHOR']; ?></b>
			</td>
			<td width="100%" height="25">
				...
				<?php } ?>
					...
					<td class="gensmall" width="100%" style="border:0;"><div style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>;">&nbsp;<b><?php echo ((isset($this->_rootref['L_POST_SUBJECT'])) ? $this->_rootref['L_POST_SUBJECT'] : ((isset($user->lang['POST_SUBJECT'])) ? $user->lang['POST_SUBJECT'] : '{ POST_SUBJECT }')); ?>:</b> <?php echo $_postrow_val['POST_SUBJECT']; ?></div><div style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_END'])) ? $this->_rootref['S_CONTENT_FLOW_END'] : ''; ?>;"><?php if ($this->_rootref['S_IS_BOT']) {  echo $_postrow_val['MINI_POST_IMG']; } else { ?><a href="<?php echo $_postrow_val['U_MINI_POST']; ?>"><?php echo $_postrow_val['MINI_POST_IMG']; ?></a><?php } ?><b><?php echo ((isset($this->_rootref['L_POSTED'])) ? $this->_rootref['L_POSTED'] : ((isset($user->lang['POSTED'])) ? $user->lang['POSTED'] : '{ POSTED }')); ?>:</b> <?php echo $_postrow_val['POST_DATE']; ?>							<?php if (! $this->_rootref['S_IS_BOT']) {  if ($_postrow_val['U_REPORT']) {  ?><a href="<?php echo $_postrow_val['U_REPORT']; ?>"><?php echo (isset($this->_rootref['REPORT_IMG'])) ? $this->_rootref['REPORT_IMG'] : ''; ?></a> <?php } if ($_postrow_val['U_INFO']) {  ?><a href="<?php echo $_postrow_val['U_INFO']; ?>"><?php echo (isset($this->_rootref['INFO_IMG'])) ? $this->_rootref['INFO_IMG'] : ''; ?></a> <?php } if ($_postrow_val['U_WARN']) {  ?><a href="<?php echo $_postrow_val['U_WARN']; ?>"><?php echo (isset($this->_rootref['WARN_IMG'])) ? $this->_rootref['WARN_IMG'] : ''; ?></a> <?php } if ($_postrow_val['U_DELETE']) {  ?><a href="<?php echo $_postrow_val['U_DELETE']; ?>"><?php echo (isset($this->_rootref['DELETE_IMG'])) ? $this->_rootref['DELETE_IMG'] : ''; ?></a> <?php } } ?>&nbsp;</div></td>
				</tr>
				</table>
			</td>
		</tr>
...
<table cellspacing="4" align="center" width="150"><tr><td class="postdetails" style="border: 0; position: relative; padding-bottom: 30px;">
			<?php if ($_postrow_val['POSTER_AVATAR']) {  ?><center><?php echo $_postrow_val['POSTER_AVATAR']; ?></center><?php } if ($_postrow_val['ONLINE_IMG']) {  ?><div style="text-align: center;"><?php echo $_postrow_val['ONLINE_IMG']; ?></div><?php } ?><br />
					<?php if ($_postrow_val['POSTER_POSTS'] != ('')) {  ?><b><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?>:</b> <?php echo $_postrow_val['POSTER_POSTS']; ?> <br /><?php } if ($_postrow_val['POSTER_FROM']) {  ?><b><?php echo ((isset($this->_rootref['L_LOCATION'])) ? $this->_rootref['L_LOCATION'] : ((isset($user->lang['LOCATION'])) ? $user->lang['LOCATION'] : '{ LOCATION }')); ?>:</b> <?php echo $_postrow_val['POSTER_FROM']; ?> <br /><?php } $_medal_count = (isset($_postrow_val['medal'])) ? sizeof($_postrow_val['medal']) : 0;if ($_medal_count) {for ($_medal_i = 0; $_medal_i < $_medal_count; ++$_medal_i){$_medal_val = &$_postrow_val['medal'][$_medal_i]; if ($_medal_val['S_HAS_MEDALS'] || $_medal_val['S_HAS_NOMINATIONS']) {  ?>

						<b><?php echo ((isset($this->_rootref['L_MEDALS'])) ? $this->_rootref['L_MEDALS'] : ((isset($user->lang['MEDALS'])) ? $user->lang['MEDALS'] : '{ MEDALS }')); ?>:</b> <?php echo $_medal_val['MEDALS_COUNT']; echo $_medal_val['MEDALS_NOMINATED']; ?> <br />
					<?php } $_medal_row_count = (isset($_medal_val['medal_row'])) ? sizeof($_medal_val['medal_row']) : 0;if ($_medal_row_count) {for ($_medal_row_i = 0; $_medal_row_i < $_medal_row_count; ++$_medal_row_i){$_medal_row_val = &$_medal_val['medal_row'][$_medal_row_i]; $_medal_col_count = (isset($_medal_row_val['medal_col'])) ? sizeof($_medal_row_val['medal_col']) : 0;if ($_medal_col_count) {for ($_medal_col_i = 0; $_medal_col_i < $_medal_col_count; ++$_medal_col_i){$_medal_col_val = &$_medal_row_val['medal_col'][$_medal_col_i]; ?>
						...
						<?php }} }} }} ?><!-- Begin Ultimate Points --><?php if ($this->_rootref['USE_POINTS']) {  ?>

						<b><?php echo ((isset($this->_rootref['L_POINTS_CASH_ON_HAND'])) ? $this->_rootref['L_POINTS_CASH_ON_HAND'] : ((isset($user->lang['POINTS_CASH_ON_HAND'])) ? $user->lang['POINTS_CASH_ON_HAND'] : '{ POINTS_CASH_ON_HAND }')); ?>:</b> 

						<?php if ($_postrow_val['POSTER_LOCK']) {  ?>

							<?php echo ((isset($this->_rootref['L_POINTS_LOCKED'])) ? $this->_rootref['L_POINTS_LOCKED'] : ((isset($user->lang['POINTS_LOCKED'])) ? $user->lang['POINTS_LOCKED'] : '{ POINTS_LOCKED }')); ?>

						<?php } else { ?>

							<?php echo $_postrow_val['POSTER_POINTS']; ?> 

							<?php if ($this->_rootref['USE_IMAGES_POINTS']) {  ?>

								<img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/money.png" width="11px" height="11px" alt="" />
							<?php } else { ?>

								<?php echo (isset($this->_rootref['P_NAME'])) ? $this->_rootref['P_NAME'] : ''; ?>

							<?php } if (! $_postrow_val['S_IS_OWN_POST'] && $this->_rootref['S_USER_LOGGED_IN']) {  ?>

								<br />
								<a href="<?php echo $_postrow_val['U_POINTS_DONATE']; ?>"><?php echo $_postrow_val['L_DONATE']; ?></a>
							<?php } else { ?>
							...
							<a href="<?php echo $_postrow_val['U_POINTS_MODIFY']; ?>"><?php echo $_postrow_val['L_MOD_USER_POINTS']; ?></a>
						<?php } ?>

						<br />
					<?php } if ($this->_rootref['USE_BANK']) {  if ($_postrow_val['BANK_ACCOUNT']) {  ?>

							<b><?php echo ((isset($this->_rootref['L_BANK_TITLE_MAIN'])) ? $this->_rootref['L_BANK_TITLE_MAIN'] : ((isset($user->lang['BANK_TITLE_MAIN'])) ? $user->lang['BANK_TITLE_MAIN'] : '{ BANK_TITLE_MAIN }')); ?>:</b>

							<?php if ($_postrow_val['POSTER_BANK_LOCK']) {  ?> 
								<?php echo ((isset($this->_rootref['L_POINTS_LOCKED'])) ? $this->_rootref['L_POINTS_LOCKED'] : ((isset($user->lang['POINTS_LOCKED'])) ? $user->lang['POINTS_LOCKED'] : '{ POINTS_LOCKED }')); ?>

							<?php } else { ?> 
								<?php echo $_postrow_val['BANK_GOLD']; ?> 
								...
								<br />
								<a href="<?php echo $_postrow_val['U_BANK_MODIFY']; ?>"><?php echo $_postrow_val['L_MOD_USER_POINTS']; ?></a>
							<?php } ?>

							<br />
						<?php } } ?><!-- End Ultimate Points --><!-- [+] Karma MOD --><?php if ($_postrow_val['S_KARMA_ENABLED']) {  if ($_postrow_val['U_KARMA_COMMENTS']) {  ?>

							<a href="<?php echo $_postrow_val['U_KARMA_COMMENTS']; ?>"><strong><?php echo ((isset($this->_rootref['L_KARMA'])) ? $this->_rootref['L_KARMA'] : ((isset($user->lang['KARMA'])) ? $user->lang['KARMA'] : '{ KARMA }')); ?>:</strong> <?php echo $_postrow_val['KARMA']; ?></a>
						<?php } else { ?>

							<strong><?php echo ((isset($this->_rootref['L_KARMA'])) ? $this->_rootref['L_KARMA'] : ((isset($user->lang['KARMA'])) ? $user->lang['KARMA'] : '{ KARMA }')); ?>:</strong> <?php echo $_postrow_val['KARMA']; ?>

						<?php } ?>

						<br />
					<?php } ?><!-- [-] Karma MOD --><?php if ($_postrow_val['S_PROFILE_FIELD1']) {  ?><!-- Use a construct like this to include admin defined profile fields. Replace FIELD1 with the name of your field. -->
					...
					<?php } ?>
						...
						<div class="postbody">
							<div style="float: right;">
								<!-- [+] Karma MOD --><?php if ($_postrow_val['S_KARMA_ENABLED']) {  if ($_postrow_val['U_KARMA_INCREASE'] || $_postrow_val['U_KARMA_DECREASE']) {  if ($_postrow_val['U_KARMA_INCREASE']) {  ?>

											<a href="<?php echo $_postrow_val['U_KARMA_INCREASE']; ?>" title="<?php echo ((isset($this->_rootref['L_KARMA_INCREASE'])) ? $this->_rootref['L_KARMA_INCREASE'] : ((isset($user->lang['KARMA_INCREASE'])) ? $user->lang['KARMA_INCREASE'] : '{ KARMA_INCREASE }')); ?>">
												<img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/Karma+.png">
											</a>
										<?php } ?>

										<br clear="all" />
										<?php if ($_postrow_val['U_KARMA_DECREASE']) {  ?>

											<a href="<?php echo $_postrow_val['U_KARMA_DECREASE']; ?>" title="<?php echo ((isset($this->_rootref['L_KARMA_DECREASE'])) ? $this->_rootref['L_KARMA_DECREASE'] : ((isset($user->lang['KARMA_DECREASE'])) ? $user->lang['KARMA_DECREASE'] : '{ KARMA_DECREASE }')); ?>">
												<img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/Menos-Karma.png">
											</a>
										<?php } } } ?><!-- [-] Karma MOD -->
							</div>
...
<?php if ($_postrow_val['SIGNATURE']) {  ?>
						...
						<br /><hr /><span class="postbody"><?php echo $_postrow_val['SIGNATURE']; ?></span>
		...
		<?php if (!($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
			...
			<td><div class="gensmall" style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>;">&nbsp;

...
class points_robbery
{
	...
	function main($id, $mode)
	{
		...
		if(isset($_POST['submit']))
		{
			...
			if ($user->data['user_last_robbery_time'] > time()) 
			{
				$msg = 'Debes esperar un poco mas para hacer tu próximo robo.<br /><br /><a href="' . append_sid("{$phpbb_root_path}points.$phpEx", "mode=robbery") . '">&laquo; ' . $user->lang['BACK_TO_PREV'] . '</a>';
				trigger_error($msg);
			} 
			else 
			{
				$sql_arr = array(
					'user_last_robbery_time' => time() + 30
				);
